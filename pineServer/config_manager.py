"""
ConfigManager - Configuration management for gamification system

Implements IConfigManager interface.
...
class ConfigManager(IConfigManager):
    """
    Configuration manager that loads from database and caches.
    """
    ...
    def _get_default_batch_config(self) -> Dict[str, Any]:
        return {"size": 10, "distribution_easy": 2, "distribution_central": 6, "distribution_hard": 2}
    
    def _get_default_scoring_config(self) -> Dict[str, Any]:
        return {"base_multiplier": 5, "participation_bonus": 10, "error_penalty": 2}
    
    def _get_default_miniboss_config(self) -> Dict[str, Any]:
        return {"min_batches_after_fail": 3, "nivel_invisible_threshold": 0.5, "min_correct_rate": 0.8}
    
    def _get_default_streak_config(self) -> Dict[str, Any]:
        return {"min_correct": 4, "max_days": 30}
